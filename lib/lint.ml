...
module Checks = struct
  ...
  let with_temp_dir prefix f =
    let dir = D.create_temp_dir prefix in
    let res = f dir in
    D.remove_dir dir;
    res

  let get_dune_project_version ~pkg url =
    with_temp_dir "lint-dune-project-version-" @@ fun dir ->
    let res =
      OpamProcess.Job.run
    ...
    | Up_to_date _ | Result _ -> (
        ...
        try
          let ch = open_in dune_project in
          try
            let content = really_input_string ch (in_channel_length ch) in
            close_in ch;
            match Sexplib.Sexp.parse content with
            | Sexplib.Sexp.Done
                (List [ Atom "lang"; Atom "dune"; Atom version ], _) ->
                Ok (Some version)
            | Done _ -> failwith "(lang dune ...) is not the first construct"
            | Cont _ -> failwith "Failed to parse the dune-project file"
          with e ->
            close_in_noerr ch;
            raise e
        with
...
let msg_of_errors (package, (err : Checks.error)) =
...
let run_lint pkg repo_dir =
  ...
  (* NOTE: We read from channel instead of file to prevent name and version
     fields being automatically added *)
  let ic = open_in opam_path in
  let opam = OpamFile.OPAM.read_from_channel ic in
  let errors = Checks.run_checks ~pkg opam in
  match errors with
  | [] -> print_endline "No errors"
  | _ ->
      errors |> List.map msg_of_errors |> List.iter print_endline;
      exit 1
